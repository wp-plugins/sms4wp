...
function sms4wp_get_authentication_header(& $auth_token) {
    ...
    // return sprintf("Authorization: token %s:%s", $email, $auth_token);
    return sprintf("Authorization: token %s", $auth_token);
}

function sms4wp_prepare_data() {

	// $backend_user_id = get_user_meta( $membership_id, 'sms4wp_backend_user_id', TRUE );
	// $user = sms4wp_admin_user_get( $sms4wp_user_id );
    // $backend_user_id = $user['membership_id'];
    $data = array(
        'today_sent'    => 0,
        'point'         => 0,
        'sms_cost'      => 0,
    ...
    //\axis_framework\includes\core\utils\axis_dump_pre($point, 'point');
    ...
    if ( is_array( $counter ) ) {
        ...
        //\axis_framework\includes\core\utils\axis_dump_pre($counter, 'counter');
    ...
    //\axis_framework\includes\core\utils\axis_dump_pre( $costs, 'costs' );
...
/*function sms4wp_admin_user_get($user_id) {
...
function sms4wp_admin_user_point() {
	...
	// return BACKEND_ADMIN_URL_ROOT . '/user_point/';
}

function sms4wp_admin_message_counter() {

    return SMS4WP_BACKEND_URL . '/message_counter/';
    // return BACKEND_ADMIN_URL_ROOT . '/message_counter/';
}
