...
if( !function_exists('sms4wp_ajax_receivers') ) {
    function sms4wp_ajax_receivers() {
	    ...
	    // $notcall = $wpdb->get_row( "SELECT count(ID) AS cnt FROM `".SMS4WP_RECEIVERS_TABLE."` WHERE ( re_use = '1' )" . $query_where ); // 수신거부 
...
if( !function_exists('sms4wp_ajax_msg_sends') ) {
	function sms4wp_ajax_msg_sends() {
	    ...
	    // $add_file1 = $_FILES['add_file1'];
	    // $add_file2 = $_FILES['add_file2'];
		...
		// die( json_encode("---" . implode('|', $receivers)) );
		...
		// die( json_encode("---" . implode(',', $groups)) );
		if ( is_array( $groups ) ) {
