...
class SMS4WP {
	...
	public function __construct( $email, $auth_token, $signature ) {
		...
		// $this->header           = array( sprintf( "Authorization: token %s:%s", $this->email, $this->auth_token ) );
	...
	public function Add( $args = array() ) {
	    ...
	    // $add_file2 = $args['bulk_file']; // 벌크이미지
	    ...
	    // move_uploaded_file( $add_file1['tmp_name'], $upload_dir['basedir'] . '/' . basename($add_file1['name']) );
	    // die( json_encode($add_file1['name'] . ' - test') );
	    // exit;
