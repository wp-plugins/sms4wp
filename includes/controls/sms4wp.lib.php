...
function sms4wp_send_message( $data = array() ) {
    ...
    // $SMS = new SMS4WP( $email, $auth_token, $signature );
...
function sms4wp_add_send_message( $args ) {
    ...
    // $result = $wpdb->query( $wpdb->prepare( "INSERT INTO `".SMS4WP_SEND_LIST_TABLE."` (`option_name`, `option_value`, `autoload`) VALUES (%s, %s, %s) ON DUPLICATE KEY UPDATE `option_name` = VALUES(`option_name`), `option_value` = VALUES(`option_value`), `autoload` = VALUES(`autoload`)", $option, $serialized_value, $autoload ) );
    return;
}
...
function sms4wp_list_groups( $args ) {
    ...
    foreach ( $rows as $group ) { 
        ...
        // $list['group'][$c]['depth'] = strlen( $list['group'][$c]['gr_order'] );
